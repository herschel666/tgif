exports.getValidatedParams = ({ sessionId, ...pathParameters }) => {
  ...
  if (!Boolean(String(sessionId).match(/^[0-9a-f]{48}$/))) {
    throw Error('The given session ID is invalid.');
  }

  return { userId: String(userId), sessionId };
};

exports.isValidSession = async ({ userId, sessionId }, ddb) => {
  const sessionEntry = await ddb.getSettingsSession(userId);

  return Boolean(sessionEntry) && sessionEntry.SessionId === sessionId;
};

const RE_TIMEZONE =
  /^(?:Africa|America|Antarctica|Asia|Atlantic|Australia|Europe|Indian|Pacific)\/[A-Za-z/_-]+$/;
